...
+ (NSString *)stringWithMilliseconds:(NSInteger)seconds;
{
	...
	if (hours > 0) {
		[string appendFormat:@"%u.", (unsigned)hours];
	}
	
	if (minutes >= 10 || hours == 0) {
		[string appendFormat:@"%u.", (unsigned)minutes];
	} else {
		[string appendFormat:@"0%u.", (unsigned)minutes];
	}
	
	if (seconds >= 10) {
		[string appendFormat:@"%u", (unsigned)seconds];
	} else {
		[string appendFormat:@"0%u", (unsigned)seconds];
	}
...
+ (NSString *)stringWithInteger:(NSInteger)integer upperRange:(NSInteger)upperRange;
{
	if (integer <= upperRange) {
		return [[self class] stringWithFormat:@"%d", (unsigned)integer];
	} else {
		return [[self class] stringWithFormat:@"%d+", (unsigned)upperRange];
	}
}
